    ...
    public class MinionStateMachine : MonoBehaviour
    {
        ...
        public class TeamShare
        {
            ...
            public TeamShare(PrisonDodgeballManager.Team team, int teamSize, int numBalls)
            {
                ...
                BallClaimByIdx = new int[NumBalls];
            ...
            public void EnsureCaches(PrisonDodgeballManager mgr)
            {
                if (BallClaimByIdx == null || BallClaimByIdx.Length != NumBalls)
                {
                    BallClaimByIdx = new int[NumBalls];
                    for (int i = 0; i < NumBalls; ++i)
                        BallClaimByIdx[i] = -1;
            ...
            public bool TryClaimBall(int ballIdx, int ownerKey)
            {
                if (ballIdx < 0 || ballIdx >= BallClaimByIdx.Length)
                    return false;

                if (BallClaimByIdx[ballIdx] == -1 || BallClaimByIdx[ballIdx] == ownerKey)
                {
                    BallClaimByIdx[ballIdx] = ownerKey;
                    return true;
                }
                return false;
            }

            public void ReleaseBallClaim(int ballIdx, int ownerKey)
            {
                if (ballIdx < 0 || ballIdx >= BallClaimByIdx.Length)
                    return;
                if (BallClaimByIdx[ballIdx] == ownerKey)
                    BallClaimByIdx[ballIdx] = -1;
            }
        }
        ...
        class CollectBallState : MinionState
        {
            ...
            public override void Enter()
            {
                ...
                {
                    hasDestBall = TeamData.TryClaimBall(destBall.Index, ownerKey);
            ...
            public override void Exit(bool globalTransition)
            {
                    if (hasDestBall)
                    {
                        TeamData.ReleaseBallClaim(destBall.Index, ownerKey);
                        hasDestBall = false;
                    }
            }
            ...
            public override StateTransitionBase<MinionFSMData> Update()
            {
                ...
                if (hasDestBall)
                {
                    ...
                    if (destBall.IsHeld || destBall.State != PrisonDodgeballManager.DodgeballState.Neutral || !destBall.Reachable
                            || TeamData.BallClaimByIdx[destBall.Index] != ownerKey)
                    {
                        TeamData.ReleaseBallClaim(destBall.Index, ownerKey);
                        hasDestBall = false;
                    }
                ...
                if (!hasDestBall)
                {
                    if (FindClosestAvailableDodgeball(out var candidate))
                    {
                        if(TeamData.TryClaimBall(candidate.Index, ownerKey))
                        {
                            destBall = candidate;
                            hasDestBall = true;
                        }
        ...
        class GlobalTransitionState : MinionState
        {
            ...
            public override StateTransitionBase<MinionFSMData> Update()
            {
                ...
                if (!Mgr.IsGameOver)
                {
                    if (Time.timeSinceLevelLoad - lastEvadeT >= 2f * Minion.EvadeCoolDownTimeSec)
                    {
                        lastEvadeT = Time.timeSinceLevelLoad;
                        Minion.FaceTowards(Mgr.TeamPrison(Team).position);
                        var ev = (Random.value < 0.33f) ? MinionScript.EvasionDirection.Brake 
                                : (Random.value < 0.5f ) ? MinionScript.EvasionDirection.Left
